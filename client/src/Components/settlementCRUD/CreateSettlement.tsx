      ...
      return (
          <>
            <form style={{ width: '100vw', height: '100vh' }} onSubmit={(e) => handleSubmit(e)}>
              ...
              <select value={payment_id} onChange={(e) => setPayment_id(data.Payments?.find(el => el.date === e.target.value)?._id)}>
              ...
              <select value={room_id} onChange={(e) => setRoom_id(data.Rooms?.find(el => el.room_number === e.target.value)?._id)}>
                <option value="">Не визначена</option>
                {roomsOpt?.map(room => (
